...
public sealed class LinqExercises
{
    ...
    public IEnumerable<string> Task01_StudentsFromWarsaw()
    {
        var query = from s in UniversityData.Students
                where s.City.Equals("Warsaw")
                    select $"{s.FirstName} {s.LastName} {s.Id} {s.City}";
        return query;
    }
    ...
    {
        var email = UniversityData.Students.Select(e=> e.Email);
        return email;
    }
    ...
    {
        var query = from s in UniversityData.Students
            orderby s.LastName descending, s.FirstName descending
    ...
    {
        return UniversityData.Enrollments.Join(UniversityData.Courses, e=>e.CourseId, c=>c.Id,
            (e,c)=> new {e.CourseId,  c})
            .Join(UniversityData.Enrollments, a=> a.CourseId, c=>c.Id, (x, 
                    c)=> $"{x.CourseId} - {c}");
    }
    ...
    public IEnumerable<string> Challenge01_StudentsWithMoreThanOneActiveCourse()
    {
        return UniversityData.Enrollments
            .Where(e => e.IsActive)
            .Join(UniversityData.Students, e => e.StudentId, s => s.Id, (e, s) => new { s.FirstName, s.LastName })
            .GroupBy(x => new { x.FirstName, x.LastName })
            .Where(g => g.Count() > 1)
            .Select(g => $"{g.Key.FirstName} {g.Key.LastName}: {g.Count()} courses");
    }
